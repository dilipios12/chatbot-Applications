...
app.post('/api/register', (req, res) => {
  ...
  // Create empty chat entry for this username
  const chatPath = path.join(__dirname, 'chat.json');
  let chatData = {};
  if (fs.existsSync(chatPath)) {
    chatData = JSON.parse(fs.readFileSync(chatPath, 'utf8'));
  ...
  if (!chatData[username]) {
    chatData[username] = {}; // username -> empty object
  }
  fs.writeFileSync(chatPath, JSON.stringify(chatData, null, 2));
...
function sendChatMsg() {
  ...
  fetch('http://localhost:3000/api/save-message', {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      username : loggedInUsername, // only one userId field
      userId,
      sender: "user",
      message: msg,
...
app.post('/api/save-message', (req, res) => {
  let { username, userId, sender, message, timestamp } = req.body;

  // if (!username || !sender || !message || !timestamp) {
  //   return res.status(400).json({ error: 'Missing fields' });
  // }
  ...
  // If no chatSessionId, create one automatically
  if (!userId) {
    userId = uuidv4();
  ...
  if (!chatData[username][userId]) {
    chatData[username][userId] = [];
  }

  chatData[username][userId].push({
    sender,
    message,
    timestamp
  });
  ...
  res.json({ success: true, userId }); // return id so frontend can reuse it
});
